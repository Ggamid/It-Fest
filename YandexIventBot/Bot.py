...
from threading import \
    Thread  # библиоткеа для Создания нескольких потоков чтобы бот одновременно мог отправлять уведомления и функционировать
...
for i in range(1, len(dict_perfom)):
    stroka += dict_perfom[i][0] + " " + dict_perfom[i][
...
def start_message(message):
    ...
    if message.text == "/start":
        ...
        last_post = types.KeyboardButton("Последний Пост 📢")

        markup.add(support, add_tag, remove_tag, on_off,
                   last_post)  # дабавляем кнопочки которые будем крипеть к клавиатуре
        ...
        bot.send_message(message.chat.id, "Пользуйся командами 🔧 в клавиатуре опишу их:  "
                                          ...
                                          "\n Последний пост 📢 - c каждым нажатием на эту кнопку, ты будешь получать все более и более старые публикации из сообщества, пока не дойдешь до конца (проверить до какой эры дойдет бот, остается за тобой 😉)."
                         )
    ...
    if message.chat.type == "private":
        ...
        if message.text == "Поддержка⚙️":  # обработка нажатия на кнопку поддержки
            ...
            if number_of_post > 40:
                ...
                bot.send_message(message.chat.id,
                                 "Вау 🤩, тебе так интересны публикации 🧐, так переходи в сообщество и смотри больше там. Вы просмотрели все 40, доступных боту публикаций 📃. Теперь при нажатии на кнопку вы начнете получать те же публикации заново 🔄")
            number_of_post += 1
            ...
            for i in list_user:
                ...
                for tag in list_tag:
                    for post in list_post:
                        if tag in list_post[post][1] and list_post[post][0] and Sqlighter.check_post_in_sent_post(i,
                                                                                                                  list_post[
                                                                                                                      post][
                                                                                                                      4]) == "Можно Отправить" and \
                                list_post[post][
                                    3]:  # выполняем проверки на хэштэг, на дату публикации на то был отправлен ли этот пост юзеру ранее или нет.
                            bot.send_message(i, list_post[post][1])  # если все проверки пройдены отправляем пост
                            Sqlighter.add_id_post_to_sent_post(i, list_post[post][
                                4])  # пост отправлен и нам нужно добавить его в отправленные посты и привязать к пользоваетлю
            time.sleep(1800)
...
#
t1 = Thread(
    target=sender)  # создаем поток чтобы программы работала не только на отправку уведомлений, но и чтобы работел бот
...
def pars(domain):
    status = session.method("wall.get",
                            {"domain": domain, "count": 50})  # запрос в vk api забираем информацию о 50 постах
...
def GetInfo(domain):  # главная функция обрабатывает данные возвращаемы функцией Pars
    ...
    for i in range(0, len(data2)):  # вытаскиваем из data2 тексты постов
        ...
        list_post_text[i] = [date_post, text_post, img_post, check_data,
                             id_post]  # словарь в котором хранятся данные о постах: дата в формате month-day, текст пост, картинка (если она есть), переменная chtck_data и id поста
